...
class RLAIFVDataset(torch_data.Dataset):
    def __init__(
        ...
        cache_dir = data_dir + '-logp'

        if not op.exists(cache_dir):
            os.makedirs(cache_dir, exist_ok=True)

        # data_path = [file for file in os.listdir(cache_dir if not op.exists(cache_dir) else cache_dir) if file.endswith('.parquet') and 'logp' in file]
        data_path = [
            file
        ...
        if len(data_path) == 0:
            ...
            # hf_data = hf_datasets.load_dataset(data_dir)['train'].select(range(20)).cast_column("image", hf_datasets.Image(decode=False))
...
class SingleDataSourceDataset(torch_data.Dataset):
    def __init__(
        self, ds_name, data_dir, tsv_filenames: List[str], intent="sft", shuffle=False
    ) -> None:
        super().__init__()
    ...
    def fetch_sample(self, file_idx, row_idx):
        ...
        assert isinstance(
            file, TSVFile
        ), f"Expecting TSVFile but get {file} as {type(file)}"
...
class MultiDataSourceDataset(torch_data.Dataset):
    def __init__(
        self,
        data_sources: List[SingleDataSourceDataset],
        data_source_weights: List[int],
        shuffle=False,
    ):
        super().__init__()
        ...
        if shuffle:
            for ds in self.ds_list:
                assert (
                    ds.shuffle
                ), f"Single dataset {ds} not shuffled, but multi-source dataset required to be shuffled"
