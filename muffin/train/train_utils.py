...
def SFT_collator_fn(instances, pad_token_id):
    ...
    if "context_ids" in instances[0]:
        image_bounds, context_ids, position_ids = tuple(
            [instance[key] for instance in instances]
            for key in ("image_bounds", "context_ids", "position_ids")
        )
        batch["image_bounds"] = image_bounds
        batch["context_ids"] = torch.nn.utils.rnn.pad_sequence(
            context_ids, batch_first=True, padding_value=0
        )
    return batch
...
def encode_multimodal_preference_sample(
    ...
    """
    编码多模态偏好样本，将源数据转换为模型训练所需的格式。

    参数:
        source (Dict): 包含对话和图像数据的源字典。必须包含 'chosen' 和 'rejected' 键，可能包含 'image' 和参考日志概率信息。
        tokenizer (transformers.PreTrainedTokenizer): 用于文本分词的分词器。
        multimodal_cfg (Dict): 多模态配置字典，包含 'image_processor'、'is_multimodal' 等配置。
        preprocess_func (Callable, optional): 自定义预处理函数，用于对对话数据进行进一步处理。如果未提供，则使用默认的 preprocess 函数。

    返回:
        Tuple[Dict, Dict]: 包含处理后的拒绝样本和选择样本的字典。
    """
    ...
    if preprocess_func is None:
        ...
        rej_data_dict = dict(
            input_ids=rej_data_dict["input_ids"][0], labels=rej_data_dict["labels"][0]
        )

        win_data_dict = preprocess([win_conv], tokenizer)
        win_data_dict = dict(
            input_ids=win_data_dict["input_ids"][0], labels=win_data_dict["labels"][0]
        )
    else:
        ...
        if "context_ids" in rej_data_dict:
            rej_data_dict = dict(
                input_ids=rej_data_dict["input_ids"][0],
...
def preprocess_v1(
    ...
    """
    预处理对话数据，将原始对话转换为模型训练所需的输入和标签格式。

    参数:
        sources (List[List[Dict]]): 对话数据，每个对话是一个消息列表，消息为字典包含 'from' 和 'value'。
        tokenizer (transformers.PreTrainedTokenizer): 用于对文本进行分词的分词器。
