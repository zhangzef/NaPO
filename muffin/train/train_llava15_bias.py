...
from muffin.data.bias_datasets_trainer import (
    LLaVA15BiasPOTrainer,
    DataCollatorForBiasPODataset,
    BiasPODataset,
)

# from muffin.data.bias_datasets_trainer_less import (
#     LLaVA15LessPOTrainer,
#     DataCollatorForLessPODataset,
#     LessPODataset,
# )

# from muffin.data.bias_datasets_nodes_trainer import (
#     LLaVA15BiasPOTrainer,
#     DataCollatorForBiasPODataset,
#     BiasPODataset,
# )
...
@dataclass
class DataArguments:
    train_mode: Optional[str] = field(default="")
...
class TrainingArguments(transformers.TrainingArguments):
    alpha: Optional[str] = field(default="-")
    use_dyn_q: Optional[str] = field(default="-")
    loss_type: Optional[str] = field(default="-")
    loss_weight: Optional[str] = field(default="-")
...
def make_dpo_data_module(tokenizer, data_args, reference_model):
    # if "less" in data_args.train_mode:
    # train_dataset = LessPODataset(
    #     tokenizer=tokenizer,
    ...
    # data_collator = DataCollatorForLessPODataset(
    #     tokenizer=tokenizer,
    #     beta=data_args.dpo_beta,
    #     train_mode=data_args.train_mode,
    # )
    # else:
    train_dataset = BiasPODataset(
        tokenizer=tokenizer,
        data_dir=data_args.data_dir,
        multimodal_cfg=dict(
            is_multimodal=data_args.is_multimodal,
    ...
    print(f"Train data size is {len(train_dataset)}", flush=True)
    data_collator = DataCollatorForBiasPODataset(
        tokenizer=tokenizer,
        beta=data_args.dpo_beta,
        train_mode=data_args.train_mode,
    )
...
def train(attn_implementation=None):
    ...
    data_args.eval_data_source_names = (
        data_args.eval_data_source_names.split("#")
    ...
    if training_args.task == "LM":
        ...
        # if "less" in data_args.train_mode:
        # trainer = LLaVA15LessPOTrainer(
        #     model=model, tokenizer=tokenizer, args=training_args, **data_module
        # )
        # else:
        trainer = LLaVA15BiasPOTrainer(
            model=model, tokenizer=tokenizer, args=training_args, **data_module
        )
...
if __name__ == "__main__":
    # torch.distributed.init_process_group(backend='nccl', init_method='env://', timeout=datetime.timedelta(seconds=18000))
