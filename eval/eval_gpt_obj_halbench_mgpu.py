    ...
    for cap in caps:
        ...
        if "The following is a response without hallucination." in cap_text:
            new_cap_text = cap_text.split(
                "The following is a response without hallucination."
            )[-1].strip()
        elif "The following is a response with hallucination." in cap_text:
            new_cap_text = cap_text.split(
                "The following is a response with hallucination."
            )[0].strip()
        elif "Generate a response without errors." in cap_text:
            new_cap_text = cap_text.split("Generate a response without errors.")[
                -1
            ].strip()
...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path, openai_apikey):
        ...
        self.chat_model = Chat(
            model="gpt-3.5-turbo-0613", timeout_sec=100, openai_apikey=openai_apikey
        )
        self.fail_limit = 100
        ...
        coco_double_words = [
            word
            for word in self.inverse_synonym_dict.keys()
    ...
    def _load_generated_captions_into_evaluator(
        self, cap_file, sample_num, org_dir=None
    ):
        """
        Meant to save time so imid_to_objects does not always need to be recomputed.
        """
        # Read in captions
        self.caps, imids, self.metrics = load_generated_captions(
            cap_file, org_dir=org_dir
        )
    ...
    def gpt_caption_processor(self, max_workers=64):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            futures = [
                executor.submit(self.get_gpt_resp, data_item) for data_item in data_list
            ]
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                ...
                try:
                    new_data_item, used_tokens, success_tokens = (
                        future.result()
                    )  # type = List
                    all_used_tokens = {
                        key: all_used_tokens[key] + used_tokens[key]
                        for key in all_used_tokens.keys()
                    }
                    all_success_tokens = {
                        key: all_success_tokens[key] + success_tokens[key]
                        for key in all_success_tokens.keys()
                    }
    ...
    def compute_chair(self, cap_file, sample_num, gpt_process=False, org_dir=None):
        ...
        self._load_generated_captions_into_evaluator(
            cap_file, sample_num, org_dir=org_dir
        )
    ...
    if cap_file.endswith(".json"):
        ...
        if "image_id" not in caps[0].keys():
            try:
                ...
                raise Exception(
                    "Expect origin test input file directory for .jsonl cap file"
                )
...
def print_metrics(hallucination_cap_dict, result_file, ckpt, quiet=False):
    ...
    if result_file != "":
        with open(result_file, "a") as f:
            f.write("\n----------------------------------\n\n")
            f.write(f"{ckpt}-objhal:\n")
            f.write("CHAIRs\tCHAIRsr\tCHAIRi\tsent_num\tcoco_num\tavg_len\n")
            f.write("%s\n" % metric_string)
            f.write("\n----------------------------------\n\n")
...
def merge_jsonl_files(input_dir, output_file):
    """
    合并指定目录下所有以 .jsonl 结尾的文件到一个新的 .jsonl 文件中。

    :param input_dir: 输入目录路径
    :param output_file: 输出合并后的 .jsonl 文件路径
    """
    # 构建匹配模式，递归查找所有子目录
    pattern = os.path.join(input_dir, "**", "*.jsonl")
    # 使用 glob.glob 查找所有符合条件的文件
    jsonl_files = glob.glob(pattern, recursive=True)

    if not jsonl_files:
        print(f"在目录 {input_dir} 下未找到任何 .jsonl 文件。")
        return

    print(f"找到 {len(jsonl_files)} 个 .jsonl 文件，开始合并...")
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as infile:
                    for line_number, line in enumerate(infile, 1):
                        ...
                        if line:  # 确保行不为空
                            try:
                                json_obj = json.loads(line)  # 验证 JSON 格式
                                json.dump(json_obj, outfile, ensure_ascii=False)
                                outfile.write("\n")
                            except json.JSONDecodeError as e:
                                print(
                                    f"警告: 文件 {file_path} 第 {line_number} 行的 JSON 格式不正确，跳过该行。"
                                )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    cap_path = os.path.join(args.cap_folder, args.cap_file)
    merge_jsonl_files(args.cap_folder, cap_path)

    print("=======load prediction=======")
    print("load imgids file:", args.cap_file)
    _, imids, _ = load_generated_captions(cap_path, org_dir=args.org_folder)
    ...
    save_dir = pathlib.Path(cap_path).absolute().parent

    print("***do process***", flush=True)
    cap_dict = evaluator.compute_chair(
        cap_path, args.sample_num, gpt_process=args.use_gpt, org_dir=args.org_folder
    )
    print(f"Done computing")

    save_hallucinated_words(cap_path, cap_dict, save_dir, sample_num=args.sample_num)
    print(f"Done Saving")
