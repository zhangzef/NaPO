...
def get_args():
    ...
    parser.add_argument(
        "--word_association", type=str, default="datasets/AMBER/data/relation.json"
    )
    parser.add_argument(
        "--safe_words", type=str, default="datasets/AMBER/data/safe_words.txt"
    )
    parser.add_argument("--cap_folder", type=str, default="")
    parser.add_argument(
        "--annotation", type=str, default="datasets/AMBER/data/annotations.json"
    )
    parser.add_argument(
        "--metrics", type=str, default="datasets/AMBER/data/metrics.txt"
    )
...
def get_inference_data(directory):
    all_data = []
        ...
        if filename.endswith(".jsonl"):
            ...
            with open(file_path, "r", encoding="utf-8") as file:
                for item in file:
                    ...
                    # for item in reader:
                    obj = json.loads(item)
                    all_data.append({"id": obj["question_id"], "response": obj["text"]})
    return all_data
...
def main(args):
    ...
    inference_data = get_inference_data(args.cap_folder)
    ...
    if dimension["g"]:
        ...
        Ha_p = round(
            100 - metrics["non_hallu_score"] / metrics["non_hallu_num"] * 100, 1
        )
    ...
    if dimension["de"] and dimension["da"] and dimension["dr"]:
        Accuracy = round(
            metrics["qa_correct_score"] / metrics["qa_correct_num"] * 100, 1
        )
        Precision = round(
            metrics["qa_ans_no_score"] / metrics["qa_ans_no_num"] * 100, 1
        )
    ...
    if dimension["de"]:
        hallucination_Accuracy = round(
            metrics["ha_qa_correct_score"] / metrics["ha_qa_correct_num"] * 100, 1
        )
        hallucination_Precision = round(
            metrics["ha_qa_ans_no_score"] / metrics["ha_qa_ans_no_num"] * 100, 1
        )
        hallucination_Recall = round(
            metrics["ha_qa_no_score"] / metrics["ha_qa_no_num"] * 100, 1
        )
    ...
    if dimension["da"]:
        attr_Accuracy = round(
            (
                metrics["as_qa_correct_score"]
                + metrics["an_qa_correct_score"]
                + metrics["aa_qa_correct_score"]
            )
            / (
                metrics["as_qa_correct_num"]
                + metrics["an_qa_correct_num"]
                + metrics["aa_qa_correct_num"]
            )
            * 100,
            1,
        )
        attr_Precision = round(
            (
                metrics["as_qa_ans_no_score"]
                + metrics["an_qa_ans_no_score"]
                + metrics["aa_qa_ans_no_score"]
            )
            / (
                metrics["as_qa_ans_no_num"]
                + metrics["an_qa_ans_no_num"]
                + metrics["aa_qa_ans_no_num"]
            )
            * 100,
            1,
        )
        attr_Recall = round(
            (
                metrics["as_qa_no_score"]
                + metrics["an_qa_no_score"]
                + metrics["aa_qa_no_score"]
            )
            / (
                metrics["as_qa_no_num"]
                + metrics["an_qa_no_num"]
                + metrics["aa_qa_no_num"]
            )
            * 100,
            1,
        )
        ...
        state_Accuracy = round(
            metrics["as_qa_correct_score"] / metrics["as_qa_correct_num"] * 100, 1
        )
        state_Precision = round(
            metrics["as_qa_ans_no_score"] / metrics["as_qa_ans_no_num"] * 100, 1
        )
        state_Recall = round(
            metrics["as_qa_no_score"] / metrics["as_qa_no_num"] * 100, 1
        )
        state_F1 = round(
            2
            * (state_Precision / 100)
        ...
        number_Accuracy = round(
            metrics["an_qa_correct_score"] / metrics["an_qa_correct_num"] * 100, 1
        )
        number_Precision = round(
            metrics["an_qa_ans_no_score"] / metrics["an_qa_ans_no_num"] * 100, 1
        )
        number_Recall = round(
            metrics["an_qa_no_score"] / metrics["an_qa_no_num"] * 100, 1
        )
        number_F1 = round(
            2
        ...
        action_Accuracy = round(
            metrics["aa_qa_correct_score"] / metrics["aa_qa_correct_num"] * 100, 1
        )
        action_Precision = round(
            metrics["aa_qa_ans_no_score"] / metrics["aa_qa_ans_no_num"] * 100, 1
        )
        action_Recall = round(
            metrics["aa_qa_no_score"] / metrics["aa_qa_no_num"] * 100, 1
        )
        action_F1 = round(
            2
    ...
    if dimension["dr"]:
        relation_Accuracy = round(
            metrics["asso_qa_correct_score"] / metrics["asso_qa_correct_num"] * 100, 1
        )
        relation_Precision = round(
            metrics["asso_qa_ans_no_score"] / metrics["asso_qa_ans_no_num"] * 100, 1
        )
        relation_Recall = round(
            metrics["asso_qa_no_score"] / metrics["asso_qa_no_num"] * 100, 1
        )
        relation_F1 = round(
            2
