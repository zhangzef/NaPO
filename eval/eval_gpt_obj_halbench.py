...
if __name__ == "__main__":
    ...
    if args.cap_folder != "":
        ...
        cap_file_list_path = [
            x for x in f_list if x.endswith(".jsonl") and args.cap_type in x
        ]
    ...
    for path in cap_file_list_path:
        ...
        temp_caps, temp_imids, _ = load_generated_captions(
            path, org_dir=args.org_folder
        )

        print("***do process***", flush=True)
        cap_dict = evaluator.compute_chair(
            path, args.sample_num, gpt_process=args.use_gpt, org_dir=args.org_folder
        )
