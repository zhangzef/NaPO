...
if __name__ == "__main__":
            ...
            try:
                ...
                try:
                    if response["model"] != chat.model:
                        ...
                        print(
                            f"Except f{chat.model}, but got message from f{real_model}",
                            flush=True,
                        )
    ...
    avg_score_str = "Average score: {:.2f}".format(sum(scores) / len(scores))
    hal_rate_str = "Hallucination rate: {:.2f}".format(
        sum(hallucination) / len(hallucination)
    )
    avg_score_each_type_str = "Average score for each question type:"
    avg_score_each_type = ",".join(
    ...
    if args.result_file != "":
        with open(args.result_file, "a") as f:
            f.write("\n----------------------------------\n\n")
            f.write(f"{args.ckpt}-mmhal:\n")
            f.write(f"{avg_score_str}\n")
            f.write(f"{hal_rate_str}\n")
            f.write(f"{avg_score_each_type_str}\n")
            f.write(f"{avg_score_each_type}\n")
            f.write("\n----------------------------------\n\n")
